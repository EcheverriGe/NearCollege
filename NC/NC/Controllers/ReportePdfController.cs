...
namespace NC.Controllers
{
    ...
    public class ReportePdfController : Controller
    {
        ...
        public ActionResult pdf()
        {
            ...
            List<Tbl_Usuarios> usuarios = db.Tbl_Usuarios.ToList();
            ...
            foreach (var item in usuarios)
            {
                ...
                _cell = new PdfPCell(new Paragraph(item.Tbl_Roles.NombreRol, fontText));
                _cell.HorizontalAlignment = Element.ALIGN_CENTER;
                table.AddCell(_cell);

                _cell = new PdfPCell(new Paragraph(item.NombreUsuario, fontText));
                _cell.HorizontalAlignment = Element.ALIGN_CENTER;
                table.AddCell(_cell);

                _cell = new PdfPCell(new Paragraph(item.ApellidoUsuario, fontText));
                _cell.HorizontalAlignment = Element.ALIGN_CENTER;
                table.AddCell(_cell);

                _cell = new PdfPCell(new Paragraph(item.CorreoUsuario, fontText));
                _cell.HorizontalAlignment = Element.ALIGN_CENTER;
                table.AddCell(_cell);
            }
    ...
    class HeaderFooter : PdfPageEventHelper
    {
        ...
        public override void OnEndPage(PdfWriter writer, Document document)
        {
            ...
            PdfPCell _cell = new PdfPCell(new Paragraph("Usuarios Registrados"));
            _cell.HorizontalAlignment = Element.ALIGN_CENTER;
            _cell.Border = 0;
